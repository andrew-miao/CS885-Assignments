...
from config import env_name, initial_exploration, batch_size, update_target, goal_score, log_interval, device, replay_memory_capacity, lr, max_epsilon, hidden_size
...
def update_target_model(online_net, target_net):
    ...
    # tau = 0.005
    # for target_param, local_param in zip(target_net.parameters(), online_net.parameters()): # <- soft update
...
def main():
    ...
    for e in range(5000):
        ...
        hidden = (torch.Tensor().new_zeros(1, 1, hidden_size).to(device), torch.Tensor().new_zeros(1, 1, hidden_size).to(device))
    ...
    plt.plot(Ep_arr)
    plt.savefig('./drqncartpole_seq-len=4_hidden-size=16_gradclip_l1smooth_update-period=120_lr=1e-03_end=0.05.png')
